...
describe('(Component) PressArticles', () => {
  context('using default props', () => {
    const subjectShallow = shallow(<PressArticles />)
    const subjectMount = mount(<PressArticles />)
  ...
  context('when we pass in our own properties', () => {
    ...
    const subject = mount(<PressArticles {...videoImages} />)
